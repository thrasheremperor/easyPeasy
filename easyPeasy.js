...
//MOVIES FUNCTIONS
let quantityMovies=movies.length;
let movieTitles=movies.map(movie=>movie.title);
let titlesInOrder=movieTitles.sort().join('\n');
let arrayOverview=movies.map(movie=>[movie.title,movie.overview]);
let stringOverview=arrayOverview.sort().map(movie=>movie.join('\n---------------\n')).join('\n\n');
let voteFilter=movies.filter(movie=>movie.vote_average>=7);
let score=voteFilter.map(movie=>movie.vote_average);
let scoreSum=score.reduce((before,after)=>after+=before);
let scoreAverage=(scoreSum/score.length).toFixed(1);
let mostVoted=voteFilter.map(movie=>[movie.title,movie.vote_average,movie.overview]);
let mostVotedList=mostVoted.map(movie=>movie.join('\n')).join('\n\n');



//THEATERS FUNCTIONS
let roomsInEach=theaters.map(theaters=>theaters.total_rooms);
let totalRoomz=roomsInEach.reduce((before,after)=>after+=before);
let arrayRooms=theaters.map(theaters=>[theaters.name,theaters.address,theaters.description]);
let arrayRoomsFinal=arrayRooms.map(theaters=>theaters.join('\n')).join('\n\n');



//FAQ FUNCTIONS
let totalFaqs=faqs.length;
let arrayFaqs=faqs.map(faqs=>[faqs.faq_title,faqs.faq_answer]);
let stringFaqs=arrayFaqs.map(faqs=>faqs.join('\n')).join('\n\n');
...
http.createServer((req,res)=>{
	...
	switch (req.url) {
		case '/':
			...
			res.write('--------\n');
            res.write('El mejor sitio para encontrar las mejores películas, incluso mucho mejor que Netflix, Cuevana y PopCorn!\n\n');
			res.write(`Total de películas en cartelera​: ${quantityMovies}\n\n`);
			res.write('Peliculas\n');
			res.write('----------\n');
            res.write(`${titlesInOrder}\n\n`);
			res.write('Pie de pagina​\n');
			res.write('--------------\n');
		...
		case '/en-cartelera':
			...
			res.write('---------------------\n');
			res.write(`Total de peliculas​: ${quantityMovies}\n\n`);
			res.write(`${stringOverview}`)
			res.end();
			break;
		case '/mas-votadas':
            res.write('MAS VOTADAS\n------------\n');
            res.write(`Total de películas mas votadas: ${voteFilter.length}\n`);
            res.write(`Rating promedio:  ${scoreAverage}\n\n\n`)
            res.write('LISTADO DE PELICULAS\n')
            res.write('----------------------\n');
            res.write(`${mostVotedList}`)
            res.end();
			break;
		case '/sucursales':
            res.write(`NUESTRAS SALAS\n`);
            res.write('----------------\n')
            res.write(`Total de salas: ${totalRoomz}\n\n`);
            res.write('LISTADO DE SALAS\n');
            res.write('------------------\n')
            res.write(`${arrayRoomsFinal}`);
			res.end();
			break;
		case '/contacto':
            ...
            res.write('Si deseas contactarnos podés escribirnos al siguiente email: dhmovies@digitalhouse.com o en las redes sociales.\n\n');
            res.write('Envianos tu consulta, sugerencia o reclamo y será respondido a la brevedad posible.\n\n');
            res.write('Recordá que también podes consultar la sección de Preguntas Frecuentes para obtener respuestas inmediatas a los problemas más comunes.');
			res.end();
			break;
		case '/preguntas-frecuentes':
            res.write('PREGUNTAS FRECUENTES\n');
            res.write('---------------------\n');
            res.write(`Total de Preguntas: ${totalFaqs}\n\n`);
            res.write('LISTADO DE PREGUNTAS\n');
            res.write('----------------------\n')
            res.write(`${stringFaqs}`)
			res.end();
			break;
		default:
